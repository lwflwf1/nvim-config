...
# Bug U fix: with pipefail, `ls` on an empty/missing dir returns 2 and poisons
# the command substitution, which set -e turns into a hard abort on FRESH
# installs (first ToolInstall loop would always die rc=2 before doing anything).
count_parsers() { ls "$DATA_DIR/site/parser"/*.so 2>/dev/null | wc -l | tr -d ' ' || true; }
count_mason()  { ls "$DATA_DIR/mason/packages" 2>/dev/null | wc -l | tr -d ' ' || true; }
EXPECTED_PARSERS=27
...
# Bug U fix: `gcc --version` proves presence, not usability — minimal images
# (e.g. Ubuntu 26.04 without libc6-dev) ship a runnable gcc with NO headers, so
# every parser build fails confusingly. Probe a real link and auto-install
# build-essential when the toolchain cannot compile.
cc_probe() { # cc_probe <cc>  -> 0 if a program that includes <stdio.h> links
    local cc="$1" out rc
    out="$(mktemp)"
    printf '#include <stdio.h>\nint main(void){return 0;}\n' | "$cc" -x c - -o "$out" >/dev/null 2>&1
    rc=$?
    rm -f "$out"
    return "$rc"
}
if ! cc_probe gcc; then
    warn "gcc runs but cannot link a test program — the C library headers are missing (libc6-dev / build-essential)"
    if [ "$USER_MODE" = 1 ]; then
        err "install build-essential (or glibc-devel on RHEL/CentOS) manually, then re-run"
        exit 1
    fi
    log "Installing build-essential (gcc/g++/make + libc headers) ..."
    pkg_install build-essential gcc g++ make || true
    if ! cc_probe gcc; then
        err "gcc still cannot compile after the install attempt; install build-essential/libc6-dev manually and re-run"
        exit 1
    fi
    ok "gcc compiles after installing build-essential"
fi
if command -v g++ >/dev/null 2>&1 && ! cc_probe g++; then
    warn "g++ cannot link a test program — parsers with C++ scanners (scanner.cc) will fail; install g++/libstdc++-dev"
fi
...
# Bug U fix: en_US.UTF-8 is not generated by default on fresh WSL images, and
# exporting it makes child processes emit a setlocale warning. Use whichever
# locale actually exists (C.UTF-8 ships with every Ubuntu/Debian).
...
while [ "$mason_count" -lt "$EXPECTED_MASON_TOOLS" ] && [ "$mason_attempt" -le 3 ]; do
    ...
    mason_count="$(count_mason)"
    mason_attempt=$((mason_attempt + 1))
done
if [ "$mason_count" -lt "$EXPECTED_MASON_TOOLS" ]; then
    warn "Mason tools installed: $mason_count/$EXPECTED_MASON_TOOLS — run :ToolInstall once in an interactive nvim session (needs network)"
else
...
parsers=$(find "$DATA_DIR/site/parser" -name '*.so' 2>/dev/null | wc -l | tr -d ' ' || true)
